        ...
        for (app,st),g in self.df.groupby(["Application_ID","Stage"]):
            try:
                ...
                out.append({
                    ...
                    "Risk_Grade":g["Risk_Grade"].iloc[-1],
                    "UW_Decision":g["UW_Decision"].iloc[-1] if "UW_Decision" in g else None,
                    "Stage_Status":g["Status_After_Activity"].iloc[-1],
                    "Performed_By":", ".join(g["Performed_By"].dropna().unique()),
                    "Issues_Count":0})
            except Exception as e: logger.error(f"Stage fail {app},{st} | {e}")
        self.stage_master=pd.DataFrame(out); logger.info(f"Stage Master rows={len(self.stage_master)}")
    def build_application_master(self):
        ...
        for app,g in self.df.groupby("Application_ID"):
            try:
                ...
                out.append({
                    "Application_ID":app,"Product_Type":g["Product_Type"].iloc[0],
                    "Channel":g["Channel"].iloc[0],"Application_Start":a0,"Application_End":a1,
                    "Total_TAT_Minutes":tat,"Age_Days":age,
                    "Final_Risk_Grade":g["Risk_Grade"].iloc[-1],
                    "Final_UW_Decision":g["UW_Decision"].iloc[-1] if "UW_Decision" in g else None,
                    "Application_Status":g["Status_After_Activity"].iloc[-1],
                    "Performed_By":", ".join(g["Performed_By"].dropna().unique()),"Issues_Count":0})
            except Exception as e: logger.error(f"App fail {app} | {e}")
        self.application_master=pd.DataFrame(out); logger.info(f"App Master rows={len(self.application_master)}")
